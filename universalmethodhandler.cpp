...
void UniversalMethodHandler::nextStep()
{
    switch (m_step) {
    case StepCheckAdb:
        logMessage("Checking ADB Server --- Ok");
    ...
    case StepSearchDevices:
        logMessage("Searching devices --- Ok");
        if (m_progress) m_progress->setValue(20);
        m_step = StepDeviceInfo;
        QTimer::singleShot(1000, this, &UniversalMethodHandler::nextStep);
        break;
    ...
    case StepDeviceInfo: {
        ...
        try {
            ...
            m_logs->appendPlainText("Device connected");
        }
        
        if (m_progress) m_progress->setValue(25);
        m_step = StepShowMessage;
        QTimer::singleShot(700, this, &UniversalMethodHandler::nextStep);
        break;
    }
    ...
    case StepInstallApk: {
        logMessage("Installing Patch File --- ");
        if (m_progress) m_progress->setValue(60);
        ...
        try {
            ...
            logMessage("Failed - method not available");
        }
        
        if (m_progress) m_progress->setValue(65);
    ...
    case StepSetDeviceOwner:
        ...
        try {
            ...
            m_logs->appendPlainText("Set DPC as device owner.");
        } catch (...) {
            m_logs->appendPlainText("Device owner command failed");
        }
    ...
    case StepStartActivity:
        ...
        try {
            ...
            m_logs->appendPlainText("Activity start failed");
        }
